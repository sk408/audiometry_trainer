...
const TestingInterface: React.FC<TestingInterfaceProps> = ({
  ...
  const stopTone = useCallback(() => {
    ...
    setPatientJustResponded(false);
  }, [patientResponse, currentStep, updateTrainerState, procedurePhase, toneActive, simulatePatientResponse]);
  ...
  const handleStoreThreshold = useCallback(() => {
    ...
    if (session) {
      ...
      // Deep-copy the step to avoid mutating the shared testSequence array
  ...
  const handleAudiogramClick = (frequency: number, level: number) => {
    ...
    // Sync the new level into the service so subsequent tone plays use the correct value
